...
constructor(
    ...
    suspend operator fun invoke(
        ...
        expectedRevision: Long,
    ): DomainResult<Review> = householdRepository.saveMember(member, focusedIncomeContributorId, expectedRevision)
}
    ...
    suspend operator fun invoke(
        ...
        val current =
            responsibilityRepository.getResponsibilities(reviewId).find { it.id == responsibilityId }
                ?: return DomainResult.failure(DomainError.NotFound("Responsibility $responsibilityId not found"))
...
constructor(
    ...
    suspend operator fun invoke(
        ...
        val validation = ResponsibilityRules.validateDetails(responsibility)
        if (!validation.isValid) {
            return DomainResult.failure(DomainError.Validation(validation.errors))
        }
...
constructor(
    ...
    suspend operator fun invoke(
        ...
        val assumptions =
            runCatching { CalculationAssumptions.fromJson(review.assumptionsJson) }
                .getOrDefault(CalculationAssumptions.Default)
            ...
            responsibilities
                ...
                .map { item ->
                    ResponsibilityIndicativeLine(
                        ...
                        ResponsibilityCalculator.indicativeAmount(item, assumptions),
                    )
                }
        val snapshot =
            CalculationSnapshot(
...
constructor(
    ...
    suspend operator fun invoke(
        ...
        val review =
            reviewRepository.getReview(reviewId)
                ?: return DomainResult.failure(DomainError.NotFound("Review $reviewId not found"))
        if (snapshotRepository.isStale(review)) {
            return DomainResult.failure(
