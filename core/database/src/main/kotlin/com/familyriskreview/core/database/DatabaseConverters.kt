...
class DatabaseConverters {
    ...
    @TypeConverter fun toPriority(value: String?): ResponsibilityPriority? =
        value?.let { ResponsibilityPriority.valueOf(it) }

    @TypeConverter fun fromTimingKind(value: TimingKind?): String? = value?.name
    @TypeConverter fun toTimingKind(value: String?): TimingKind? =
        value?.let { TimingKind.valueOf(it) }
}
